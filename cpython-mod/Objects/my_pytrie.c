...
PyMethodDef PyTrie_methods[] = {
  ...
  {"search", (PyCFunction)PyTrie_search, METH_VARARGS,
   "Devuelve true si la palabra completa está en el trie y marcada como terminal."},
...
static PySequenceMethods trie_sequence_methods = {
    .sq_concat = 0,
    .sq_repeat = 0,
    .sq_item = 0,
    .sq_ass_item = 0,
    .sq_length = trie_len,
    .sq_contains = trie_contains,
    .sq_inplace_concat = 0,
    .sq_inplace_repeat = 0
};
...
static PyMethodDef MyModMethods[] = {
    {"print_trie_test", print_trie_test, METH_NOARGS, "Imprimir para test"},
    {NULL, NULL, 0, NULL}
};

static struct PyModuleDef my_mod_module = {
    PyModuleDef_HEAD_INIT,
    "my_mod",                   
    "Módulo AST personalizado",
    -1,
    MyModMethods
};
...
static PyTrieNode *
py_trie_node_create(Py_UCS4 ch)
{
...
int
PyTrie_init(PyTrieObject *self, PyObject *args, PyObject *kwds)
{
...
static int
finish_insert(PyTrieObject *self, const char *str)
{
  ...
  /* iteramos sobre todos los carácteres, creando un nodo si no tiene uno para su respectiva letra */
  for(size_t i=0;str[i] != '\0';i++)
    {
      PyObject *children;
      if(current == NULL) children = self->root;
      else children = current->children;
      ...
      if (! value)
	{
	  ...
	  /* encapsulamos */
	  PyObject *new_cap = PyCapsule_New(new, "PyTrieNode", py_trie_node_capsule_destructor);
	  if (!new_cap)
	    {
...
static const char *
extract_one_arg_str(const char *fn_name, PyObject *args)
{
  PyObject *obj;
...
PyTrieNode *
is_in_trie(PyTrieObject* self, const char *str)
{
  if (str[0] == '\0')
    {
      return 0;
...
PyObject *
PyTrie_starts_with(PyObject* self, PyObject* args)
{  
  const char *str = extract_one_arg_str("insert", args);
...
PyTrie_delete(PyObject* self, PyObject *args)
{
  ...
  if (!path || !keys) {
    ...
    if (!current || !current->is_terminal) goto not_found;
    ...
    for (Py_ssize_t i = depth - 1; i >= 0; i--) {
        ...
        if (!node || node->is_terminal || PyDict_Size(node->children) > 0) break;

        PyObject *key = PyList_GetItem(keys, i);
        PyObject *parent_dict = (i == 0) ? trie->root :
	  ((PyTrieNode *)PyCapsule_GetPointer(PyList_GetItem(path, i - 1), "PyTrieNode"))->children;
...
static int build_trie_lines_from_dict(PyObject *dict, PyObject *lines, 
    ...
    // Recogemos las claves en una lista para poder ordenarlas y saber el último
    ...
    for (Py_ssize_t i = 0; i < nkeys; i++) {
        ...
        if (node->children && PyDict_Check(node->children)) {
            if (build_trie_lines_from_dict(node->children, lines, new_prefix, last_child) < 0) {
                Py_DECREF(keys);
                return -1;
            }
        }
...
PyObject *trie_str(PyObject *self) {
    ...
    if (trie->root && PyDict_Check(trie->root)) {
      if (build_trie_lines_from_dict(trie->root, lines, "", 1) < 0) {
        ...
        PyObject *err_line = PyUnicode_FromString("└── <error al recorrer trie>");
        if (err_line) {
